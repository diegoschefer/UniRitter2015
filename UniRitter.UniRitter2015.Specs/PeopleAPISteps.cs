...
namespace UniRitter.UniRitter2015.Specs
{
    ...
    public class PeopleAPISteps
    {
        ...
        public void ThenIReceiveAMessageListingAllValidationErrors()
        {
            ...
            Assert.That(validationMessage, Contains.Substring("O campo firstName é obrigatório."));
            Assert.That(validationMessage, Contains.Substring("O campo email não é um endereço de email válido."));
        }
        ...
        {
            personData = new Person{
                id = Guid.Parse("7ebb0c42-0f97-4990-81ea-a02f69650731"),
                firstName = "Fulano",
                lastName = "de Tal",
                email = "fulano@gmail.com",
                url = "http://fulano.com.br"
            };
            Assert.NotNull(personData.id);
        }

        [Given(@"a valid update message to that resource")]
        public void GivenAValidUpdateMessageToThatResource()
        {
            Assert.That(personData.id, Is.EqualTo(personData.id));
        }
